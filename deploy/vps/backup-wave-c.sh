#!/bin/sh
# Wave C pre-implementation backup. Copy only. No deletes. No n8n postgres restart.
set -e
STAMP=pre-automation-v2-wave-c-20260822-0945
...
dst = "/opt/backups/pre-automation-v2-wave-c-20260822-0945/homestead/homestead.sqlite"
...
docker exec n8n_postgres pg_dump -U n8nuser -Fc -d n8n -f /tmp/n8n-wave-c.dump
docker cp n8n_postgres:/tmp/n8n-wave-c.dump "$DIR/n8n/n8n.dump"
ENTRIES=$(docker exec n8n_postgres pg_restore -l /tmp/n8n-wave-c.dump | grep -c "^[0-9]" || true)
docker exec n8n_postgres rm -f /tmp/n8n-wave-c.dump
...
if docker exec -u node n8n_n8n n8n export:workflow --all --output=/tmp/all-workflows.json; then
  ...
  python3 - <<'PY'
import json
raw = open("/opt/backups/pre-automation-v2-wave-c-20260822-0945/n8n/workflow-export/all-workflows.json","r",encoding="utf-8").read()
data = json.loads(raw)
print("N8N_WORKFLOW_COUNT", len(data) if isinstance(data, list) else 1)
hs = [w.get("name") for w in (data if isinstance(data, list) else [data]) if str(w.get("name","")).startswith("HOMESTEAD")]
print("HOMESTEAD_WORKFLOWS", len(hs))
PY
  echo "N8N_EXPORT=OK"
else
